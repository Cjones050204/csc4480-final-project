...
-- Refresh gradebook letter grades
update gradebook gb
    set finalLetterGrade = (
        select
            case
                when sg.finalGrade >= 0.94 then 'A'
                when sg.finalGrade >= 0.90 then 'A-'
                when sg.finalGrade >= 0.87 then 'B+'
                when sg.finalGrade >= 0.84 then 'B'
                when sg.finalGrade >= 0.80 then 'B-'
                when sg.finalGrade >= 0.77 then 'C+'
                when sg.finalGrade >= 0.74 then 'C'
                when sg.finalGrade >= 0.70 then 'C-'
                when sg.finalGrade >= 0.67 then 'D+'
                when sg.finalGrade >= 0.64 then 'D'
                when sg.finalGrade >= 0.60 then 'D-'
                else 'F'
            end
        from studentGrades sg
        where sg.studentID = gb.studentID and sg.CRN = gb.CRN
    );
